        ...
        items.map((item) =>
            ctx.cache.tryGet(item.link, async () => {
                ...
                try {
                    ...
                    // }
                     //筛选有字幕和高清标签的磁力
                     const suburl = magnets.find(obj => obj.lablelength > 1);
                     //console.log(suburl);
                     //筛选有高清标签的磁力
                     const gaoqingurl = magnets.find(obj => obj.gaoqing === "高清");
                     //console.log(gaoqingurl);
 
                     if (suburl) {
                         //判断是否有字幕和高清标签的磁力
                         item.enclosure_url = suburl.link;
                     } else if (gaoqingurl) {
                         //判断是否高清标签的磁力
                         item.enclosure_url = gaoqingurl.link;
                     };
                     item.enclosure_type = 'application/x-bittorrent';
                } catch (e) {
                    // no-empty
