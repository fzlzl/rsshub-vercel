...
//获取重定向后的链接地址函数
async function getRedirectedHost() {
  try {
    const responsess = await got('https://user.301go.net:8899/?u=http://taohuazu.com/&p=/', {
      followRedirect: true,
    });

    return responsess.redirectUrls[0];
  } catch (error) {
    console.log(error.responsess.body);
  }
}

//发送请求
module.exports = async (ctx) => {
    const linkUrl = await getRedirectedHost();//调用获取重定向后的链接地址函数
    console.log('Redirected linkUrl:', linkUrl);
    const host = `${linkUrl}forum.php?mod=forumdisplay&fid=181&filter=typeid&typeid=770`;
    console.log('Redirected host:', host);
